...
- Admin URL : https://github.com/Jammomu/RestaurantAdmin
- Server URL : https://github.com/Jammomu/YS_TeamProject_Bobjo
- Test ID : 관리자  
- Test PW : 1234
...
막막하기만 했던 프로젝트 작업에 대하여, 프로젝트가 전반적으로 어떻게 진행 되는지 알게되었습니다.

프로젝트를 진행하면서 회원가입 / 로그인을 구현하면서 Spring Security와 JWT의 개념에 대해 이해하고 

공부할 수 있는 시간이 되었습니다.

구현 하고자 했던 모든 부분을 다 해내지는 못했지만, 팀원들과 협력하여 부족한 부분을 채워나가는 법을 배우고, 

앞으로의 공부와 프로젝트를 함에 있어서 내가 어떻게 해야하는지를 배울 수 있는 좋은 경험이 되었습니다.
...
유저 ID를 파라미터로 보내 정보 CRUD를 사용했었는데

토큰에 담긴 정보로 사용하면 불필요한 파라미터를 사용 하지 않을 수 있다는걸 알게 되었고,

처음 SQL문을 설계하여 개발 도중 SQL문과 API가 추가되어 가독성과 성능면에서 모두 불필요한 코드를 짜게 되었는데

다음부터는 설계부터 깊은 고민과 시간을 투자하여 불필요한 코드삭제와 성능 개선에 대해
...
카카오페이, 다음 지도 API 등 여러 라이브러리를 사용 할 수 있어서 좋았고

결제 예약 등 할 수 있는 기능이 늘어 좋은 프로젝트 였습니다.

<br>

### 😎 윤지현

직접 api를 만들어 프론트와 백을 연결하면서, 효율적으로 사이트를 구축하는 방법에 대해 많이 

고민하게 되는 시간이었습니다.

처음 기획단계에서 사용자 중심의 로직을 생각하고 구현한다는게 어려웠지만,

기능을 하나씩 완성해 나가면서 직관적이고 실용적인 페이지를 만들었습니다.
